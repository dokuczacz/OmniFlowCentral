...
These are the files referenced by the PrawoL prompt (“files attached”) and kept under `docs/`:
1. `docs/QUERY_DATASET.md` — Query contract + examples (“query data”).
2. `docs/eli.json` — ELI dataset descriptor.
3. `docs/commonCourts.json` — Common courts reference data.
4. `docs/CUSTOM_GPT_KNOWLEDGE.json` — Canonical Custom GPT policy (allowed tools + params).
5. `docs/openapi_tools_call.yaml` — OpenAI function call schema (tools contract).
...
Current version: 2026-01-09 (attachments aligned to PrawoL prompt; golden suite verified locally)
